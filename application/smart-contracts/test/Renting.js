...
contract("Renting", function() {
    let owner;
    let accounts;
    console.log("Did you remove addTestData() in IdentityOracle.sol constructor?");
    beforeEach(async() => {
    ...
    it("should be possible to query an oracle for all roles", async () => {
      ...
      result = await renting.isKnownParticipant.call(accounts[1], 2);
      ...
      result = await renting.isKnownParticipant.call(accounts[3], 5);
      assert.equal(result, true, "accounts[3] should be a Device.");
      result = await renting.isKnownParticipant.call(accounts[4], 5);
      ...
      result = await renting.isKnownParticipant.call(accounts[6], 5);
    ...
    it("should be possible to create a RentalAgreement", async () => {
      ...
      await renting.createRenting(tenant, signature, device, usageFee, contractTerm);
    ...
    it("should only be possible to query your own RentalAgreement", async () => {
      ...
      try{
        const data = await renting.getByID.call(0, {from: accounts[4]});
        assert(false);
      }
      catch(err){
          assert.include(String(err),'revert','should throw an revert error');
    ...
    it("should be possible to get your own RentalAgreement IDs", async () => {
      ...
      await renting.createRenting(tenant, signature1, device, usageFee, contractTerm);
      await renting.createRenting(tenant, signature2, accounts[4], 2, contractTerm);
      await renting.createRenting(tenant, signature3, accounts[5], 3, contractTerm);
      await renting.createRenting(accounts[2], signature4, accounts[6], usageFee, contractTerm);
    ...
    it("should be possible to check payments as corresponding device", async () => {
      ...
      await renting.createRenting(tenant1, signature1, device, usageFee, contractTerm);
      await renting.createRenting(tenant2, signature2, accounts[4], usageFee, contractTerm);
      ...
      await renting.accept(1, signature4, {from: accounts[2]});
      ...
      result = await renting.getPaymentHash.call(timestamp1, accounts[3], accounts[1], usageFee);
      ...
      result = await renting.getPaymentHash.call(timestamp2, accounts[4], accounts[2], usageFee);
      ...
      result = await renting.getPaymentHash.call(timestamp3, accounts[4], accounts[2], usageFee);
      ...
      await renting.payForUsage(1, timestamp2, {from: accounts[2], value: usageFee});
      await renting.payForUsage(1, timestamp3, {from: accounts[2], value: usageFee});

      let balance2 = await web3.eth.getBalance(accounts[2]);
      assert.equal(balance1 > balance2, true, "New balance incorrect.");

      result = await renting.checkUsagePayment.call(0, paymentHash1, {from: accounts[3]});
